...
import { generateStudyNotes, hasGeminiKey, describeGeminiFailure } from "@/lib/gemini";
...
export async function POST(request: Request) {
  ...
  try {
    const output = await generateStudyNotes({
      prompt: buildPrompt(
        input,
        body.lectureTitle,
    ...
    return NextResponse.json({
      ...
      styleNoteCount: notes.length,
      mode: "fallback",
      warning: `Gemini request failed (${describeGeminiFailure(error)}). Returned offline demo notes instead.`,
    });
  }
}
